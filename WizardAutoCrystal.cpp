...
WizardAutoCrystal::WizardAutoCrystal() : Module("WizardAutoCrystal", "AutoCrystal By Mapwelo", Category::COMBAT, 0x0) {
	addSlider<float>("Target Range", "NULL", ValueType::FLOAT_T, &targetRange, 5.f, 20.f);

	addBoolCheck("Wizard Place", "Placed By TechHook", &autoPlace);
	...
	addBoolCheck("Wizard Break", "NULL", &autoBreak);
	addSlider<float>("Break Range", "range", ValueType::FLOAT_T, &breakRange, 3.f, 10.f);
	addSlider<float>("Max Break Damage", "maxdamage why would u want ts", ValueType::FLOAT_T, &maxBreakDame, 1.f, 20.f);
	addSlider<float>("Min Break Damage", "Damage", ValueType::FLOAT_T, &minBreakDame, 1.f, 20.f);
	addSlider<int>("Break Delay", "Delay2", ValueType::INT_T, &breakDelay, 0, 20);
	addBoolCheck("ID Predict", "IDpredict", &idPredict);

	addSlider<int>("Packets", "Packets", ValueType::INT_T, &packets, 1, 30);
	addSlider<int>("Ticks", "Ticks", ValueType::INT_T, &Ticks, 0, 100);
	addSlider<int>("Send Delay", "Delay", ValueType::INT_T, &sendDelay, 0, 20);
	addBoolCheck("Count crystal", "Crystal speed", &Crystalcounter);
	addBoolCheck("Render Damages", "Renders Damage", &Damagerender);
	addBoolCheck("Testing", "For testing", &Mob);

}
std::string WizardAutoCrystal::getModName() {

	return names;

}

bool WizardAutoCrystal::sortCrystalByTargetDame(WizardStruct a1, WizardStruct a2) {
	return a1.TgDameTake > a2.TgDameTake;
}

bool WizardAutoCrystal::sortEntityByDist(Actor* a1, Actor* a2) {
...
bool WizardAutoCrystal::isHoldingCrystal() {
...
void WizardAutoCrystal::cleardalist() {
	entityList.clear();
	targetList.clear();
	placeList.clear();
	breakList.clear();
...
int dam;
void WizardAutoCrystal::onEnable() {
	cleardalist();
	placerot = false;
	breakrot = false;
	placing = false;
...
void WizardAutoCrystal::onDisable() {
	cleardalist();
	placerot = false;
	breakrot = false;
	placing = false;
...
bool WizardAutoCrystal::canPlaceCrystal(Vec3<int> placePos) {
...
void WizardAutoCrystal::generatePlacement(Actor* target) {
	auto* lp = mc.getLocalPlayer();
	auto* region = lp->dimension->blockSource;
		...
		for (int y = -7; y <= 7; y++) {
			for (int z = -radius; z <= radius; z++) {
				...
				if (canPlaceCrystal(blockPos)) {
					WizardPlacement placement(blockPos, target);
	...
	std::sort(placeList.begin(), placeList.end(), WizardAutoCrystal::sortCrystalByTargetDame);
}


void WizardAutoCrystal::getCrystalActorList(Actor* target) {
	auto* localPlayer = mc.getLocalPlayer();
	std::vector<WizardBreaker> localBreakList;

	for (auto* ent : entityList) {
		if (ent->getEntityTypeId() != 71 || ent->stateVectorComponent->pos.dist(localPlayer->stateVectorComponent->pos) > breakRange)
			continue;

		WizardBreaker cBreaker(ent, target);
...

	{
		...
		std::sort(breakList.begin(), breakList.end(), WizardAutoCrystal::sortCrystalByTargetDame);
	}
}

void WizardAutoCrystal::placeCrystal() {
	std::lock_guard<std::mutex> lock(placeListMutex);
	...
	if (placeDelayTick >= placeDelay) {
		for (WizardPlacement& placement : placeList) {
...
void WizardAutoCrystal::breakCrystal() {
	std::lock_guard<std::mutex> lock(breakListMutex);
...
void WizardAutoCrystal::breakIdPredictCrystal() {
	std::lock_guard<std::mutex> lock(breakListMutex);
...
void WizardAutoCrystal::onNormalTick(Actor* actor) {
	cleardalist();
	auto* localPlayer = mc.getLocalPlayer();
	...
	std::sort(targetList.begin(), targetList.end(), WizardAutoCrystal::sortEntityByDist);
	float distance = 0;
	std::string namess;
...
void WizardAutoCrystal::onSendPacket(Packet* packet, bool& shouldCancel) {
...
void WizardAutoCrystal::onImGuiRender(ImDrawList* d) {
