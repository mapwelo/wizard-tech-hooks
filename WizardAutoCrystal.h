...
class WizardPlacement : public WizardStruct {
public:
	Vec3<int> placePos;
	WizardPlacement(Vec3<int> blockPos, Actor* target) {
...
class WizardBreaker : public WizardStruct {
public:
	Actor* crystalActor;
	WizardBreaker(Actor* endCrystalActor, Actor* target) {
		auto* lp = mc.getLocalPlayer();
...
class WizardAutoCrystal : public Module {
private:
	std::string names;
	std::vector<Actor*> formodnames;
	std::vector<WizardPlacement> placeList;
	std::vector<WizardBreaker> breakList;
